...
두 단계에 앞서 **해석**이 있다. `mole-core`의 실행 명령은 먼저 `Project.kt`·`Module.kt`·`Tasks.kt`를 설정 파일로 읽어 **컴파일 범위를 결정**한다. 요청한 태스크 주소에서 출발해 그 `Tasks.kt`가 참조하는 모듈, 그 모듈의 `Module.kt`가 의존하는 모듈을 따라가며 A에서 컴파일할 모듈·타깃·소스셋과 B에서 컴파일할 `src/mole`을 고른다. `./mole server/domain:jvm:test`는 `server/domain`과 그 의존 모듈의 `jvm` 타깃만 컴파일하고, `./mole ci`는 루트 `Tasks.kt`가 import 한 모듈 전부를 컴파일한다. 해석은 파일을 실행하지 않으므로 여기서 나는 오류는 전부 `ModuleDefError`다.
...
- 실행 명령은 먼저 `Project.kt`·`Module.kt`·`Tasks.kt`를 설정 파일로 해석해 컴파일 범위를 정한다. 요청한 태스크에 닿지 않는 모듈·타깃은 컴파일하지 않는다.
...
| `ModuleDefError`     | A    | 4    | 설정 파일 해석 실패, 모듈 루트 `*.kt` 컴파일·실행 실패, 패키지≠경로, 타입 불일치, 순환 의존, 의존성 해석 실패 |
