...
```
`spring-boot-autoconfigure` 모듈의 `org.springframework.boot.autoconfigure.jdbc.EmbeddedDatabaseConnection`를 보면, 테스트용 메모리 DB로 H2도 사용할수 있는 것으로 보이지만, 테스트용 JDBC 모듈로 H2(`h2database`)를 사용하면 `DataSource` 빈을 생성하지 못하는 위의 에러가 발생한다.