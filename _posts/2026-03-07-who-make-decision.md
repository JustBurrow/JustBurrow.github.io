...
3. 클라우드 서비스 시기에는 애플리케이션 개발자가 구조를 결정한다.
   ...
   - DB 쿼리는 ORM의 자동 매핑이 대체하고, 쿼리 튜닝은 ORM의 엔티티 캐시가 대체했다.
...
- 네트워크를 캡슐화 하는 기술이 늘어난다.
   ...
   2. GraphQL은 단순히 직렬화/역직렬화 하는 기술이 아니라, 클라이언트에 서버가 관리하는 데이터의 연관관계를 따라가 필요한 특정 정보에 접근하는 방법을 통제할 수 있는 방법을 제공한다.
