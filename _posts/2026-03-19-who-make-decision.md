...
- **아키텍처 결정권자의 변화** — 가장 값비싼 자원을 다루는 사람이 결정권을 쥐어왔다.
- **서버 애플리케이션 개발자가 결정권자가 된 이유** — 비즈니스와 프론트엔드 양쪽의 변화에 대응하면서도 데이터 스키마 일관성을 유지해야 하는 구조적 위치가, 서버 개발자에게 높은 추상화 능력을 요구했다.
- **다음 결정권자는 누구인가** — 기업 전체의 데이터 스키마를 설계하고 진화시킬 수 있는 사람이 될 것이다.

> 이 글에서 **스키마**는 RDB 테이블 정의에 한정하지 않고, 서비스가 다루는 데이터 전체의 구조와 관계를 의미한다.
