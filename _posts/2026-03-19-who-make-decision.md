...
- 결과적으로 일반적인 웹서비스에서 프론트 개발자는 백엔드 개발자의 코드를 이해하기(읽고 동작을 구체적인 동작을 예측하기) 매우 어렵지만, 백엔드 개발자는 비교적 프론트 개발자의 코드를 비교적 쉽게 이해할 수 있다.
...
3. 클라우드 서비스 시기에는 애플리케이션 개발자가 구조를 결정한다.
   - 2010년대 초반 ~ 현재
   - DB가 저장소 하드웨어를 HDD에서 SSD로 바꾸고, DB 서버를 포함해 서버가 대용량 메모리를 사용할 수 있게 되면서 SQL 튜닝을 포기하더라도 충분한 성능을 낼 수 있게 되면서 상대적으로 중요도가 낮아졌다.
   ...
   - DB 쿼리는 ORM의 자동 매핑이 대체하고, 반복적인 쿼리 호출은 ORM의 엔티티 캐시(1st/2nd Lv. cache)가 완화했다.
     ...
     - 이런 수단으로 충분한 성능을 낼 수 없다면, ORM 사용 자체가 잘못된 선택일 수 있다.
   ...
   - IaC 개념 보급으로 인프라 관리가 소스코드 작성으로 대체됐다.
     - 클라우드 서비스 제공자가 CLI를 제공하면서 쉘 스크립트나 프로세스 호출 코드로 작성할 수 있다.
     - Kubernetes 같은 컨테이너 클러스터는 설정 코드와 컨테이너 서비스의 관계를 프로그래밍 언어와 런타임과 같은 관계로 만들었다.
