...
3. 기업은 최적화된 클라이언트를 찾기 위해 동시에 여러 클라이언트와 서비스를 운영하게 될 것이다. 이는 고객 세그먼트에 따라 달라질 수도 있다.
4. 전체 데이터 스키마에 대해 서로 다른 서브셋을 사용하는 API를 관리해야 한다. API를 정의하고 구현하는 코드를 서버와 클라이언트 용으로 따로 관리하려면 비용이 커진다.
5. 전체 데이터 스키마를 몇 개의 그룹으로 나누고, 각 그룹에서 `GraphQL`, `gRPC` 같은 언어를 사용해 서버 구현과 클라이언트 API 모듈을 제공할 수 있다.
6. 전체 데이터 스키마에 대해 충분히 잘 모듈화한 API와 라이브러리가 있다면 클라이언트 개발은 필요한 API 라이브러리 선택과 UI 구현으로 정의할 수 있다.
7. ORM이 엔티티 캐시를 사용하듯, 서버가 공유 캐시를 사용하듯 API 라이브러리도 로컬 캐시/미디어나 기기 센서 등 기기 전용 기능을 활용하는 것까지 확장할 수도 있다.
...
9. 서버-클라이언트의 물리적 구분이 백엔드-프론트엔드의 구분이던 전통적 구분이 힘을 잃고 UI와 나머지를 뜻하는 말로 바뀔 것이다.
   - 애초 단일 하드웨어 시스템에서 UI와 그외 영역을 나누던 것이 프론트엔드와 백엔드의 구분이던 것이 인터넷 보급으로 클라이언트와 서버 구분을 뜻하는 걸로 바뀌긴 했다.