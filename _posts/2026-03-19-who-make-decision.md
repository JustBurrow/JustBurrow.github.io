> **삭제 예정. see [2026-03-21-who-make-decision.md](2026-03-21-who-make-decision.md)**
...
결정권자가 바뀐 데는 공통된 패턴이 있다. 시기마다 가장 희소하고 값비싼 자원이 달랐고, 그 자원을 다루는 전문성의 ROI가 가장 높았기 때문에 결정권도 따라 이동했다.
...
2. 닷컴 버블 ~ 가상서버(VMWare 등) 시기에는 미들웨어 개발자가 구조를 결정했다.
   ...
   - 다수의 서버가 하나의 DB를 공유하는 구조에서 DB는 서비스 전체의 단일 장애점(SPOF)이 됐다. WAS와 네트워크 설정이 안정화된 후에는 DB 성능과 가용성이 서비스 품질을 결정하게 되어, DBA의 ROI가 가장 높아졌다.
...
3. 클라우드 서비스 시기에는 애플리케이션 개발자가 구조를 결정한다.
   ...
   - WAS와 네트워크 관리가 자동화되면서 이 영역 전문가들의 ROI가 낮아졌고, 뒤이어 ORM과 하드웨어 성능 향상으로 DBA의 ROI도 낮아졌다.
   ...
   - 하드웨어 비용 하락과 이러한 도구들의 보급으로 DB 쿼리 최적화에 투자하는 ROI가 낮아졌다. 사용자가 직접 경험하는 기능을 개발해 반응을 받는 것이 DB 성능을 끝까지 짜내는 것보다 ROI가 높아졌고, 그 결과 애플리케이션 개발이 투자의 중심으로 이동했다.
...
- 아키텍처 결정권자는 현재 구현의 복잡도뿐만 아니라 장기 유지보수 복잡도와 TCO를 함께 고려해야 한다. 스키마 마이그레이션, 연동 비용, 기술 부채는 초기 설계 결정에 의해 크게 좌우되며 시간이 지날수록 누적된다.
...
- 영속화된 데이터 관리 로직은 법적 책임이 따르기 때문에 프론트엔드와 달리 시행착오보다 신중한 설계가 요구된다.
- 잘못된 백엔드 아키텍처 결정은 데이터 마이그레이션, 보안 사고, 법적 분쟁으로 이어져 수정 비용이 매우 높다. 반면 프론트엔드 결정의 수정 비용은 상대적으로 낮다. 이 비대칭성이 백엔드 아키텍처 의사결정에 더 높은 비중을 두게 만든다.
...
- 서버와 다른 플랫폼의 클라이언트에서 실행되는 라이브러리를 서버 개발자가 직접 개발할 수 있는 멀티플랫폼 기술이 성숙하고 있다.
  1. Kotlin Multiplatform, Dart(Flutter) 같은 기술을 사용하면 서버 개발자가 작성한 코드가 iOS, Android, 웹 등 여러 플랫폼에서 동시에 실행될 수 있다.
  2. 서버 개발자가 클라이언트용 API 라이브러리를 각 플랫폼별로 별도 개발하지 않고 제공할 수 있게 되어, 클라이언트 라이브러리 개발 비용이 낮아진다.
- AI 기반 코드 생성 도구의 등장으로 UI 개발 비용이 낮아지고 있다.
  1. UI는 눈에 보이는 결과물이기 때문에 AI 생성 도구의 효과가 직관적으로 드러나며, 관련 도구 발전이 집중되고 있다.
  2. 반면 영속화된 데이터 관리 로직은 법적 책임으로 인해 AI 자동화가 제한적이며, 사람의 직접 검토가 필요한 영역으로 남는다.
...
2. 그러므로 기업은 여러 클라이언트를 동시에 운영하며 최적의 사용자 경험을 찾게 될 것이다.
...
5. API 라이브러리는 로컬 스토리지(SharedPreferences, SQLite, SecureStorage 등)와 백그라운드 동기화를 담당하고, 클라이언트가 백그라운드에서 획득한 데이터를 서버로 전송하는 역할까지 백엔드 영역으로 흡수될 수 있다.
   - ORM이 엔티티 캐시로 DB 접근을 최적화하듯, API 라이브러리도 로컬 스토리지를 활용해 네트워크 접근 횟수와 지연을 줄일 수 있다.
   - 플랫폼별 스토리지와 백그라운드 처리를 캡슐화하는 멀티플랫폼 라이브러리가 성숙하면서, 서버 개발자가 클라이언트 플랫폼을 직접 이해하지 않아도 라이브러리를 제공할 수 있게 되고 있다.
   - 이 구조가 성숙하면 "언제, 어떤 스토리지에 저장하고 언제 서버로 보낼지"는 라이브러리가 판단하고, 클라이언트 코드는 데이터의 의미와 표현에만 집중하게 된다.
6. 현대 클라이언트 애플리케이션의 MVVM 아키텍처에서 프론트엔드와 백엔드의 경계가 재정의될 것이다.
   - View와 ViewModel은 UI 렌더링과 화면 상태 관리를 담당하는 명확한 프론트엔드 영역으로 남는다.
   - Model의 통신 레이어, 로컬 캐시, 데이터 동기화 로직은 API 라이브러리를 통해 서버 개발자가 제공하면서 백엔드 영역으로 재분류된다.
   - 결과적으로 프론트엔드/백엔드의 경계는 물리적 위치(클라이언트 기기/서버)가 아닌 관심사(UI/UX 표현 vs 데이터 접근과 비즈니스 로직)로 재정의된다. 직군 구분과 팀 구성도 이에 따라 달라진다.
7. 기업 전체의 데이터 스키마를 설계하고 진화시킬 수 있는 사람이 아키텍처 결정권자가 될 것이다.
   - 클라우드와 IaC의 보편화로 인프라 관리 전문성은 더 이상 결정권의 핵심 요소가 아니다.
   - 비즈니스 프로세스의 각 단계(영업, 마케팅, 운영, 고객 서비스 등)는 서로 다른 데이터 관점을 요구하면서도 동일한 원천 데이터를 참조해야 한다. 이 다양한 요구를 하나의 일관된 스키마로 충족시키는 능력이 데이터 중복, 정합성 오류, 연동 비용을 줄여 TCO를 낮춘다.
   - 데이터 스키마는 기업의 비즈니스 모델 그 자체이자 핵심 조직 구조를 결정한다. 전체 데이터 스키마를 설계하고 진화시키는 능력이 사업 전략을 기술로 번역하는 능력이 된다.
