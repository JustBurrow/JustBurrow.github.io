...
4. 전체 데이터 스키마에 대해 서로 다른 서브셋을 사용하는 API를 관리해야 한다. API를 정의하고 구현하는 코드를 서버와 클라이언트 용으로 따로 관리하려면 비용이 커진다.
   - 클라이언트 종류가 늘어날수록 각 클라이언트에 맞는 REST 엔드포인트를 별도로 유지하는 방식은 지속 가능하지 않다.
   - 스키마 변경 시 서버와 클라이언트 코드를 동기화하는 비용이 팀 규모와 클라이언트 수에 비례해서 커진다.
   - 이 문제를 해결하기 위해 API 계약(contract)을 단일 소스로 관리하고, 서버/클라이언트 코드를 자동 생성하는 방식으로 전환하는 압력이 생긴다.

5. 전체 데이터 스키마를 몇 개의 도메인 그룹으로 나누고, 각 그룹에서 `GraphQL`, `gRPC` 같은 기술을 사용해 서버 구현과 클라이언트 API 모듈을 함께 제공할 수 있다.
...
6. 전체 데이터 스키마에 대해 충분히 잘 모듈화한 API와 라이브러리가 있다면 클라이언트 개발은 필요한 API 라이브러리 선택과 UI 구현으로 정의할 수 있다.
...
7. API 라이브러리는 네트워크 통신을 넘어 로컬 캐시, 오프라인 동작, 미디어나 기기 센서 등 기기 전용 기능을 활용하는 것까지 확장할 수도 있다.
...
8. 그렇게 된다면 프론트엔드는 UI/UX 영역으로 의미가 축소되고 나머지 영역(MVVM의 Model ~ 서버)이 백엔드 영역으로 확장된다.
...
9. 서버-클라이언트의 물리적 구분이 백엔드-프론트엔드의 구분이던 전통적 구분이 힘을 잃고 UI와 나머지를 뜻하는 말로 바뀔 것이다.
