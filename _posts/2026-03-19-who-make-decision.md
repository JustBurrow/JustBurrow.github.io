...
- 프론트엔드(UI/UX)는 눈에 보이는 결과물로 인해 누구나 자기가 판단할 수 있다고 착각하기 쉽기 때문이다. 그래서 경험적으로 프론트엔드 개발자는 기술적 성숙도를 평가받기 어렵고 속도만으로 평가받는 경향이 강하다. 비전문가가 프론트엔드 개발자와 디자이너를 구분하길 기대하는 건 현실적으로 무리이고, 관리자는 어찌됐든 자기가 결정하려고 하기 쉽기 때문이다.
