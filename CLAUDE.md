...
전체 리뷰가 필요한 경우, 어시스턴트는 반드시 개별 에이전트를 직접 호출하는 대신 `editor`를 실행해야 한다.

어시스턴트는 반드시 `master`로 pull request를 열기 전에 `security-reviewer`를 (직접 또는 `editor`를 통해) 실행해야 한다.

`security-reviewer`가 🔴 차단 항목을 반환하는 경우, 어시스턴트는 해당 문제가 해결될 때까지 pull request를 열어서는 안 된다.

에이전트가 결과를 반환하는 경우, 어시스턴트는 반드시 결과를 사용자에게 제시하고 수정 작업 전에 지시를 기다려야 한다.

어시스턴트는 사용자 검토 없이 여러 에이전트의 출력을 하나의 수정에 일괄 반영해서는 안 된다.
